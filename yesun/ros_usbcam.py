...
def bounding_callback(msg):
	...
	for idx, box in enumerate(bbox):
		...
		bounding_list.append([box_class, box_xmin, box_ymin, box_xmax, box_ymax])
...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		...
		#warp_xymaxmin = np.array([xmax,ymin,1], np.float32)
		#warp_xyminmax = np.array([xmin,ymax,1], np.float32)
		#warp_xymax = np.array([xmax,ymax,1], np.float32)

		#warp_xymin = np.matmul(np_matrix,warp_xymin)
		#warp_xmax = np.matmul(np_matrix,warp_xymaxmin)
		#warp_ymin = np.matmul(np_matrix,warp_xyminmax)
		...
		#cv2.circle(img_transformed,(int(warp_xmax[0]),int(warp_ymin[1])),5,(122,122,0),-1)
		#cv2.circle(img_transformed,(int(warp_xmin[0]),int(warp_ymax[1])),5,(122,122,0),-1)
		#cv2.circle(img_transformed,(int(warp_xymax[0]),int(warp_xymax[1])),5,(122,122,0),-1)
		#print("warp_xmin",warp_xymin)

		if (len(bounding_list)!=0):
			print("bounding_list**************",bounding_list[0][0])
			for i in range (0,len(bounding_list)):
				if (bounding_list[i][0]=='yellow cone'):
					mid_x = (bounding_list[i][1]+bounding_list[i][3])/2
					mid_y = (bounding_list[i][2]+bounding_list[i][4])/2
				elif (bounding_list[i][0]=='blue cone'):
					mid_x = (bounding_list[i][1]+bounding_list[i][3])/2
					mid_y = (bounding_list[i][2]+bounding_list[i][4])/2
